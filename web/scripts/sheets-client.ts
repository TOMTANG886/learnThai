...
export async function getSheetsClient() {
  ...
  if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
    try {
      ...
      throw new Error(
        '[sheets-client] Failed to parse GOOGLE_SERVICE_ACCOUNT_JSON: ' +
    ...
    if (fromVars) {
      ...
      try {
        ...
        throw new Error(
          `[sheets-client] Failed to read credential file at ${credPath}: ` +
            (err as Error).message
        );
      }
    }
...
export async function fetchWorksheet(
  ...
  while (true) {
    try {
      ...
      throw new Error(
        `[sheets-client] Failed to fetch worksheet "${worksheetName}": ` +
          (err as Error).message
      );
    }
  }
